<div>
    ...
    <div x-data class="fixed top-20 right-0 left-0 sm:top-20 sm:right-5 sm:left-auto z-50">
        <template x-for="[id, value] in Object.entries($store.toast.toasts)" :key="id">
            <div x-show="value.show" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-90" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform translate-x-72">
